...
function main() {
    ...
    local baseName
    baseName="$(basename "$geneTreesInputPath")"
    local gtCleaned="${tmpDir}/${baseName}.cleaned"
    local consensusTreePath="${tmpDir}/${baseName}.cons"
    ...
    # Cleanup intermediate files to avoid flooding root directory.
    rm -f "$gtCleaned" "${consensusTreePath}.greedy.tree" "$consensusTreePath" || true
}
