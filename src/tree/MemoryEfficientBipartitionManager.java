...
/**
 * Memory-efficient bipartition manager that reduces memory usage from O(n²k) to
 * O(nk).
 * 
...
public class MemoryEfficientBipartitionManager {
    ...
    private static RangeBipartition.HashFunction getDefaultHashFunction() {
        return ENABLE_DOUBLE_HASHING ? new RangeBipartition.DoubleHashFunction()
                : new RangeBipartition.SumHashFunction();
    }
    ...
    /**
     * Initialize the left-to-right ordering for each gene tree (inorder traversal
     * of leaves).
     */
    ...
    /**
     * Calculate prefix sums and XORs over hashed taxon IDs for efficient range hash
     * computation.
     ...
     * Uses a combination of multiplications and XOR operations for good
     * distribution.
     */
        ...
        for (int i = 0; i < actualThreads; i++) {
            ...
            Threading.execute(() -> {
                try {
                    ...
                    System.out
                            .println("Thread " + threadId + " completed processing " + (endIdx - startIdx) + " trees");
     ...
     * Uses expensive equality checks if enabled, otherwise trusts hash function
     * uniqueness.
     */
    private void convertToFrequencyMap() {
        System.out.println("Converting unique range bipartitions to frequency map...");
        System.out.println("Expensive equality checks: "
                + (ENABLE_EXPENSIVE_EQUALITY_CHECKS ? "ENABLED" : "DISABLED (trusting hash)"));
        ...
        if (ENABLE_EXPENSIVE_EQUALITY_CHECKS) {
            ...
            // Fast approach: trust the hash function, just take the first from each hash
            // group
            for (Map.Entry<Object, List<RangeBipartition>> entry : hashToBipartitions.entrySet()) {
                List<RangeBipartition> ranges = entry.getValue();
                totalRanges += ranges.size();
    ...
    /**
     * Check if two range bipartitions are equal by comparing their actual taxon
     * sets.
     */
    private boolean rangesAreEqual(RangeBipartition range1, RangeBipartition range2) {
        ...
        // Check both orientations: {left1, right1} == {left2, right2} OR {left1,
        // right1} == {right2, left2}
    ...
    public String getStatistics() {
        ...
        sb.append("  Equality checking mode: ")
                .append(ENABLE_EXPENSIVE_EQUALITY_CHECKS ? "EXPENSIVE" : "FAST (trust hash)").append("\n");
    ...
    // Getters for accessing internal data structures (needed for memory-optimized
    // weight calculation)
