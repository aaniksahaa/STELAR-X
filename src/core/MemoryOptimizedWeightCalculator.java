 ...
 * 1. Range-based bipartition representations from
 * MemoryEfficientBipartitionManager
 * 2. Inverse index arrays for O(min(|A|, |B|)) intersection calculations
 * 3. Direct range processing without BitSet conversion
 * 
 * Memory usage: O(nk) instead of O(n²k) compared to traditional BitSet
 * approach.
 * 
...
public class MemoryOptimizedWeightCalculator {
    ...
    private Map<RangeBipartition, Double> calculateWeightsSingleThread(List<RangeBipartition> candidates) {
        ...
        // Use direct calculation for candidates (small number) and range-based for gene
        // trees (large number)
    ...
    private Map<RangeBipartition, Double> calculateWeightsMultiThread(List<RangeBipartition> candidates) {
        ...
        int numThreads = Config.getEffectiveThreadCount();

        Threading.startThreading(numThreads);

        // Use hybrid approach: direct calculation for candidates, range-based for gene
        // trees
            ...
            Threading.execute(() -> {
                try {
                    ...
                    if (startIdx >= endIdx || startIdx >= candidates.size()) {
                        System.out.println(
                                "Thread " + threadId + " skipped - invalid range [" + startIdx + ", " + endIdx + ")");
                        return;
                    }

                    System.out.println(
                            "Thread " + threadId + " processing candidates " + startIdx + " to " + (endIdx - 1));
                    ...
                    System.out.println(
                            "Thread " + threadId + " completed processing " + (endIdx - startIdx) + " candidates");
     ...
     * This replaces the traditional BitSet-based GPU approach with O(nk) memory
     * usage.
     */
    ...
    private Map<RangeBipartition, Double> calculateWeightsHybridGPU(
        ...
        try {
            ...
            WeightCalcLib.CompactBipartition[] candidateArray = (WeightCalcLib.CompactBipartition[]) candidateTemplate
                    .toArray(numCandidates);
            ...
            // Convert gene tree ranges to GPU compact format using contiguous memory
            // allocation
            WeightCalcLib.CompactBipartition geneTreeTemplate = new WeightCalcLib.CompactBipartition();
            WeightCalcLib.CompactBipartition[] geneTreeArray = (WeightCalcLib.CompactBipartition[]) geneTreeTemplate
                    .toArray(numGeneTreeBips);
     ...
     * - Format: memory[tree * numTaxa + taxon] = position (or -1 if taxon not in
     * tree)
     * - Memory layout: [tree0_taxon0, tree0_taxon1, ..., tree1_taxon0,
     * tree1_taxon1, ...]
     */
    ...
    private Memory flattenInverseIndex() {
        ...
        // Note: Memory object will be automatically freed by JNA when no longer
        // referenced
    ...
    private Memory flattenOrderings() {
        ...
        System.out
                .println("  Padding overhead: " + (paddedPositions / (double) (numTrees * paddedNumTaxa) * 100) + "%");
